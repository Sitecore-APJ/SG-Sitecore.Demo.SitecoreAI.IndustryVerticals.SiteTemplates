    ...
    <section className={`component footer relative ${props.params.styles} overflow-hidden`} id={id}>
        ...
        <div className="container grid gap-12 py-16 lg:grid-cols-[1fr_3fr] lg:py-20">
          <div className="flex flex-col gap-6">
            ...
            <RichText
              ...
              className="text-background/80 [&_p]:text-background/80 text-sm leading-relaxed"
            />
          </div>
          <div className="grid gap-10 sm:grid-cols-3 lg:grid-cols-5 lg:gap-6 xl:gap-10">
            {sections.map(({ key, title, content }) => (
              <div key={key}>
                <div className="text-background mb-5 text-sm font-semibold">{title}</div>
                <div className="[&_a]:text-background/75 [&_a]:hover:text-secondary space-y-3 [&_a]:text-sm [&_a]:transition-colors">
                  {content}
                </div>
              </div>
            ))}
      ...
      <div className="border-border border-t bg-white">
        <div className="container flex items-center justify-between py-6 max-sm:flex-col max-sm:items-start max-sm:gap-6">
          ...
          <div className="text-foreground flex items-center justify-between gap-12 text-sm max-lg:gap-8 max-sm:order-1 max-sm:flex-col max-sm:items-start max-sm:gap-4">
            <Link
