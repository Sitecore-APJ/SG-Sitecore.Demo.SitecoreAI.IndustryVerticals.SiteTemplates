        ...
        {results.map((item, index) => {
          ...
          return (
            <div className="flex flex-col items-center justify-start 2xl:w-80" key={index}>
              {/* Image */}
              <div className="bg-accent-secondary mb-7 flex h-20 w-20 items-center justify-center rounded-full">
                <Image field={image} />
              </div>
              {/* Title and Description */}
              <div className="flex flex-col items-center justify-center">
                <div className="mb-2 leading-0.5">
                  <Text tag="h5" className="text-foreground font-bold" field={title} />
                </div>
    ...
    <FeatureWrapper props={props}>
      <div className="container grid grid-cols-1 gap-4 py-24 md:grid-cols-2 lg:grid-cols-3">
        {results.map((item, index) => {
          ...
          return (
            <div
              className="border-border group hover:border-accent hover:bg-accent bg-background cursor-pointer rounded-sm border p-6 transition-colors"
              key={index}
            >
              {/* Generated Number */}
              <h1 className="text-accent group-hover:text-background mb-2 text-6xl leading-20 font-extrabold">
                {generateIndexes(index)}
              </h1>
              {/* Title and Description */}
              <div>
                <div className="text-foreground group-hover:text-background mb-3 text-xl leading-7 font-bold">
                  <Text field={title} />
                </div>
    ...
    <FeatureWrapper props={props}>
      <div className="outline-non container grid grid-cols-1 gap-12 md:grid-cols-2 lg:grid-cols-3">
        {results.map((item, index) => {
          ...
          return (
            <div key={index}>
              <div className="bg-background-surface mb-7 aspect-4/3 w-full overflow-hidden rounded-sm">
              ...
              <p className="text-foreground-light mt-2 text-base leading-relaxed">
                <Text field={description} />
              </p>
            </div>
          );
