...
const ArticleItemCard = ({ className = '', article }: ArticleItemCardProps) => {
  ...
  return (
    ...
    >
      <ArticleCard.Root
        key={article.id}
        className={`group border-border hover:shadow-primary/10 relative overflow-hidden rounded-2xl border shadow-sm hover:shadow-md hover:transition-all hover:duration-300 hover:ease-linear ${className}`}
      >
        <div className="bg-background-surface h-50 w-full overflow-hidden rounded-t-2xl">
          <Image
            src={validImageUrl}
