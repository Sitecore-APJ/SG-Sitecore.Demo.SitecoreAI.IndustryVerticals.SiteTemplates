...
const ArticleHorizontalItemCard = ({ className = '', article }: ArticleCardItemCardProps) => {
  ...
  return (
    ...
    >
      <ArticleCard.Root
        key={article.id}
        className={`group border-border bg-background relative my-4 flex max-h-52 w-full flex-row flex-nowrap rounded-2xl border p-6 transition-shadow hover:shadow-sm ${className}`}
      >
        {validImageUrl && (
