...
const ArticleHorizontalItemCard = ({ className = '', article }: ArticleCardItemCardProps) => {
  ...
  return (
    ...
    >
      <ArticleCard.Root
        key={article.id}
        className={`group border-border hover:shadow-accent/15 bg-background relative my-4 flex max-h-52 w-full flex-row flex-nowrap border p-6 shadow-sm transition-shadow hover:shadow-md ${className}`}
      >
        {validImageUrl && (
        ...
        <div className="grow flex-col pl-4">
          ...
          <div className="text-foreground bg-background-accent absolute top-4 right-4 px-2.5 py-0.5 text-xs font-semibold tracking-[0.06em] uppercase">
            {article.type}
          </div>
        </div>
      </ArticleCard.Root>
