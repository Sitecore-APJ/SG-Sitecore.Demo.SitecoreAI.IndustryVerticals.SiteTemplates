...
const ArticleItemCard = ({ className = '', article }: ArticleItemCardProps) => {
  ...
  return (
    ...
    >
      <ArticleCard.Root
        key={article.id}
        className={`group border-border hover:shadow-accent/15 relative border shadow-sm hover:shadow-md hover:transition-all hover:duration-300 hover:ease-linear ${className}`}
      >
