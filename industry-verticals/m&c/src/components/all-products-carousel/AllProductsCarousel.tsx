  ...
  return (
    <div className={`component all-products-carousel py-5 ${params.styles}`} id={id}>
      <div className="container flex flex-col items-center gap-10 text-center">
        {/* Category Filter */}
        <div className="border-border flex flex-wrap justify-center border p-1 text-xs leading-8 tracking-[0.14em] uppercase">
          {categories.map((category) => (
            <button
              className={`!text-foreground px-6 py-2 transition-colors ${selectedCategory === category ? 'bg-foreground text-background!' : 'hover:bg-background-muted'}`}
