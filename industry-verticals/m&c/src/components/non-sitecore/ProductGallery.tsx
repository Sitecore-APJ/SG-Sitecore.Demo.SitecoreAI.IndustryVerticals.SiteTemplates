        ...
        {images.map((img, idx) => {
          ...
          return (
            <button
              ...
              className={`bg-background-muted focus:ring-foreground size-15 overflow-hidden border border-transparent focus:ring-1 focus:outline-none xl:size-18 ${
                isActive
                  ? 'border-foreground cursor-not-allowed opacity-50'
                  : 'hover:border-foreground hover:border'
              }`}
            >
      ...
      <div className="grow">
        {images[mainImageIndex] ? (
          <ContentSdkImage
            ...
            className="bg-background-muted aspect-square w-full rounded-none object-contain p-4"
          />
        ) : (
          <div className="bg-background-muted aspect-square w-full rounded-none" />
        )}
      </div>
    </div>
  );
