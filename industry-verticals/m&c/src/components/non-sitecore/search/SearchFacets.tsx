...
const PriceFacet = ({ min, max }: PriceFacetProps) => {
  return (
    ...
    >
      <RangeFacet.Track className="bg-background-accent relative h-0.75 grow">
        <RangeFacet.Range className="bg-foreground absolute h-full" />
      </RangeFacet.Track>
      <RangeFacet.Start className="hover:bg-foreground hover:text-background bg-background border-border block size-5 cursor-pointer border text-center text-[10px] leading-5 shadow-none">
        {(value) => <span className="absolute top-7.5 left-0 text-sm">${value}</span>}
      </RangeFacet.Start>
      <RangeFacet.End className="hover:bg-foreground hover:text-background bg-background border-border block size-5 cursor-pointer border text-center text-[10px] leading-5 shadow-none">
        {(value) => <span className="absolute top-7.5 left-0 text-sm">${value}</span>}
      </RangeFacet.End>
    </SearchResultsFacetValueRange>
  );
    ...
    >
      {facets.map((f) => (
        <AccordionFacets.Facet
          facetId={f.name}
          key={f.name}
          className="border-border bg-background mb-4 block rounded-none border p-6 shadow-none"
        >
