...
const IconDropdown = ({
  ...
  <Popover>
    <PopoverTrigger
      className="text-foreground hover:text-foreground-light data-[state=open]:text-accent transition-colors"
      aria-label={label}
    >
      {icon}
    </PopoverTrigger>
...
export const Default = (props: NavigationIconsProps): JSX.Element => {
  ...
  return (
    <>
      <div className={`component navigation-icons ${props?.params?.styles?.trimEnd()}`} id={id}>
        <div className="flex items-center gap-3 p-4 lg:gap-5 [.component.header_&]:justify-end [.component.header_&]:px-0">
          <button
            ...
            className="hover:text-foreground-light text-foreground p-2 transition-colors"
          >
      ...
      {isSearchOpen && (
        <div className="border-border bg-background absolute top-full right-0 left-0 z-50 border-b shadow-none">
