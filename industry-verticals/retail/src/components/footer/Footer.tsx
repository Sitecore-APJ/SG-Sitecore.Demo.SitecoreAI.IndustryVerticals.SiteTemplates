  ...
  return (
    <section className={`component footer relative ${props.params.styles} overflow-hidden`} id={id}>
      <div className="bg-background-muted">
        <div className="container grid gap-10 py-16 lg:grid-cols-[1fr_3fr] lg:py-20">
          ...
          <div className="grid gap-10 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 xl:gap-8">
            {sections.map(({ key, title, content }) => (
              <div key={key}>
                <div className="text-accent mb-5 text-base font-bold">{title}</div>
      ...
      <div className="border-border bg-background border-t">
        <div className="container flex items-center justify-between py-6 max-sm:flex-col max-sm:items-start max-sm:gap-6">
          <div className="text-foreground-light text-sm max-sm:order-2">
            <Text field={props.fields.CopyrightText} />
          </div>
          <div className="text-foreground-light flex items-center justify-between gap-12 text-sm max-lg:gap-8 max-sm:order-1 max-sm:flex-col max-sm:items-start max-sm:gap-4">
            <Link
