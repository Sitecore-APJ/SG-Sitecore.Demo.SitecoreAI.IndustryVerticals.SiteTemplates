...
export const Pagination = ({
  ...
  setCurrentPage,
  windowSize = 3,
  paginationButtonClasses = 'px-3 py-2 md:px-4 md:py-3 rounded-sm',
}: PaginationProps) => {
    ...
    <div className="mt-12 flex items-center justify-center space-x-2 text-sm md:space-x-4">
        ...
        <>
          <button
            ...
            className={`${paginationButtonClasses} ${
              currentPage === 1 ? 'bg-primary text-background' : 'bg-background-accent'
            }`}
          >
            1
          </button>
      ...
      {visiblePages.map((page) => (
        <button
          ...
          className={`${paginationButtonClasses} ${
            page === currentPage ? 'bg-primary text-background' : 'bg-background-accent'
          }`}
        >
          {page}
        </button>
      ...
      {endPage < totalPages && (
        <>
          ...
          <button
            ...
            className={`${paginationButtonClasses} ${
              currentPage === totalPages ? 'bg-primary text-background' : 'bg-background-accent'
            }`}
          >
            {totalPages}
          </button>
