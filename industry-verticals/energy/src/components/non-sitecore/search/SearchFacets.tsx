  ...
  return (
    ...
    >
      <RangeFacet.Track className="bg-background-accent relative h-0.75 grow rounded-full">
        <RangeFacet.Range className="bg-primary absolute h-full rounded-full" />
      </RangeFacet.Track>
      <RangeFacet.Start className="hover:bg-primary bg-background border-border block size-5 cursor-pointer rounded-full border text-center text-[10px] leading-5 shadow-sm focus:shadow-lg">
        {(value) => <span className="absolute top-7.5 left-0 text-sm">${value}</span>}
      </RangeFacet.Start>
      <RangeFacet.End className="hover:bg-primary bg-background border-border block size-5 cursor-pointer rounded-full border text-center text-[10px] leading-5 shadow-sm focus:shadow-lg">
        {(value) => <span className="absolute top-7.5 left-0 text-sm">${value}</span>}
      </RangeFacet.End>
    </SearchResultsFacetValueRange>
  );
            ...
            {f.name !== 'price' ? (
              <AccordionFacets.ValueList className="mt-2 flex list-none flex-col space-y-2">
                {f.value.map((v, index: number) => (
                  ...
                  >
                    <AccordionFacets.ItemCheckbox className="form-checkbox hover:border-primary aria-checked:bg-background-accent border-border h-5 w-5 flex-none cursor-pointer rounded border transition duration-500 ease-in-out">
                      <AccordionFacets.ItemCheckboxIndicator className="text-primary size-5">
                        <CheckIcon />
